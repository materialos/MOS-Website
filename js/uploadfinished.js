...
if (/To git@github.com:autocontribute\/(\w+)\.git/i.test(txt)) {
	repo = txt.match(/To git@github.com:autocontribute\/(\w+)\.git/i)[1];
};
if (/Switched to a new branch '([\w\s\(\)\-\_,]+)'/i.test(txt)) {
...
if (! typeof branch === 'undefined' && ! typeof repo === 'undefined') {
	branchregex = new RegExp(branch + "(?!\\s+\\w+\\])", "gi");
	branchcommitregex = new RegExp("(\\[(?=" + branch + ")([\\w\\s\\(\\)\-\\_\\\\]+)\\s+(\\w+)\\])", "i");
	stringregex = new RegExp("('(?!" + branch + ")[\\w\\s\\-\\_/,\\(\\)]+')", "gi");
	document.getElementById('console').innerHTML = txt.replace(stringregex, "<span class='purple-text'>$1</span>").replace(/(\* \[new branch\])/gi, "<span class='green-text'>$1</span>").replace(/((The file )(.+)( has been uploaded.))/gi, "<span class='green-text'>$2</span><span class='blue-text'>$3</span><span class='green-text'>$4</span>").replace(/(\d+ file(s)? changed)/gi, "<span class='orange-text'>$1 </span>").replace(/(\d+ insertion(s)?\(\+\))/gi, "<span class='green-text'>$1 </span>").replace(/(\d+ deletion(s)?\(\-\))/gi, "<span class='red-text'>$1 </span>").replace(/(create mode [\w\s\-\_\/,\(\)\.]+)/gi, "<span class='green-text'>$1</span>").replace(/(https:\/\/github.com\/materialos\/\w+\/pull\/\d+)/gi, "<a href='$1' class='blue white-text round-2' target='_blank'>$1</a>").replace(/(HEAD is now at (\w+)([\w\s\(\)\-\_,!\.\?]+))/gi, "<a href='https://github.com/materialos/" + repo + "/commit/$2' class='blue white-text round-2' target='_blank'>$1</a>").replace(/((fatal:(?!\sremote upstream already exists\.)|error:|Error creating pull request: Unprocessable Entity \(HTTP 422\)|Permission denied|sh: 1: \.\/gitcreatepr\.sh: not found)[\w\s\-\_\/\(\)\.,]+)/gi, "<span class='red round-2'>$1</span>").replace(/((warning:|fatal: remote upstream already exists)[\w\s\-\_\/\(\),\.]+)/gi, "<span class='yellow darken-2 round-2'>$1</span>").replace(branchregex, "<a href='https://github.com/autocontribute/Concepts/tree/" + branch + "' target='_blank' class='blue white-text round-2'>" + branch + "</a>").replace(branchcommitregex, "<a href='https://github.com/materialos/" + repo + "/commit/$3' class='blue white-text round-2' target='_blank'>$1</a>").replace(/(\w{7})\.\./gi, "<a href='https://github.com/materialos/" + repo + "/commit/$1' class='blue white-text round-2' target='_blank'>$1</a>..").replace(/\.\.(\w{7})/gi, "..<a href='https://github.com/materialos/" + repo + "/commit/$1' class='blue white-text round-2' target='_blank'>$1</a>").replace(/(From (https:\/\/github.com\/materialos\/\w+))/gi, "From <a href='$2' class='blue white-text round-2' target='_blank'>$2</a>");
} else {
	// Post Error Higlights
	stringregex = new RegExp("('[\\w\\s\\-\\_/,\\(\\)]+')", "gi");
	document.getElementById('console').innerHTML = txt.replace(stringregex, "<span class='purple-text'>$1</span>").replace(/((The file )(.+)( has been uploaded.))/gi, "<span class='green-text'>$2</span><span class='blue-text'>$3</span><span class='green-text'>$4</span>").replace(/((Sorry)[\w\s\-\_\/\(\)\.,]+)/gi, "<span class='red round-2'>$1</span>");
};